...
- **Verified source checkpoint:** `7927ac3c372d4c359b07e0bedab64e09db96a5a3` — single-query rewrite preview/apply and destination validation.
...
| M6 Task5, current atomic unit only | Single-query `Rewrite`, `finishRewrite`, and existing focused tests committed at `7927ac3c372d4c359b07e0bedab64e09db96a5a3`. Independent bounded source review approved with zero findings; fresh affected package checks pass. **Task5 as a whole is not accepted.** |
...
**Finish M6 Task5 before adapters.** Continue from the existing single-query implementation and [Task5 brief](/Users/jacobdelgado/repos/spl-toolkit/.worktrees/remaining-milestones/.superpowers/sdd/milestone-6-implementation-plan/task-5-brief.md). Remaining owned paths include `verify_test.go`, `batch.go`, `batch_test.go`, and `testdata/rewrite/cases.json`, plus necessary changes in the three checkpoint files. Complete the negative internal-edit/proof seam cases, independently reviewed full-report expectations and final corpus, ordered batch handling/atomic errors, mixed-dialect and safe-plus-ambiguous results, ownership/concurrent checks, and required affected/race verification. Obtain the full immutable Task5 review. The bounded checkpoint review does not replace it.

Then hold source/index/build mutation for the coordinator's independent **Go-only acceptance window**. Preserve successful `go-results-task5.json` as an immutable baseline. Only after that window is accepted may M6 Task6 CLI/HTTP, Task7 native/Python/package interfaces and Task8 corpus/docs/full verification proceed. Finish separate broad review and final CLI/HTTP/native/installed/Python acceptance before closing M6.
...
- Three previously prepared private M6 acceptance files are missing. Their exact expected hashes are in the bundle manifest: `spl-toolkit-root-m6-go.go`, `spl-toolkit-root-m6-validation-cases.json`, and `spl-toolkit-root-m6-python-acceptance.py`. No hash-matching copy was found in the known preparation archives. **Recover them from retained task/session artifacts if possible. Otherwise document the gap and independently review replacement acceptance inputs against the approved contract before any acceptance run. Do not derive expected outputs from the implementation and call that the old baseline.**
...
3. Establish a qualified M6 lead/Task5 worker with exact remaining ownership, or continue protected work locally if no qualifying worker is available. Finish and review Task5, then reserve the unchanged-source coordinator window. Do not start adapters early.
4. Before that acceptance window, inspect [manifest.json](/Users/jacobdelgado/repos/spl-toolkit/.worktrees/remaining-milestones/_build_plan/continuation/2026-09-12/manifest.json). Verify archived input hashes and restore needed temporary paths only when missing or when the existing bytes match. Recover the three missing acceptance inputs using their recorded hashes, or explicitly review replacements without laundering old expectations. Reconcile actual public signatures before execution. The prepared core launcher is `spl-toolkit-root-m6-core-attempt.py WORKTREE ACCEPTED_HEAD`; it expects restored private inputs and the recorded Python/Go runtime paths.
