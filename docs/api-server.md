...
The local impact corpus improved from 12/1/35 to 14/16/13 for syntax-complete cases, semantic-complete selected target stages, and non-macro gaps. It does not use whole-query completeness as the success metric. The toolkit does not execute SPL, evaluate regular expressions, compare result rows, model acceleration, load macro definitions, or certify runtime compatibility. The local evidence also does not prove authorization or upstream support.
