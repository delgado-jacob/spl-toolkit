...
Exact source-bound consuming fields are direct requirements. Indeterminate, wildcard, and dynamic consumers are conditional and produce gaps. Query-derived fields remain in the parent analysis but are omitted from requirements. Create and output definitions, rename targets, removals, null tests, and query-local unavailable fields are also omitted. A rename source can still be required because it is a source-bound read. Exact direct knowledge-object references are required; wildcard or dynamic identities are conditional when the analyzer can state a defensible identity. Each exact macro invocation produces one direct macro requirement and a source-located unresolved-expansion gap. Branch children keep their direct requirements and analysis evidence; the unmodeled merge leaves the parent output environment uncertain and does not copy child fields into it. The operation does not expand a macro, lookup, data model, dataset, or other knowledge object to infer transitive requirements.
...
| 3 | Query status incomplete or, for `requirements`, requirement coverage incomplete |
...
| SPL2 | 114 | 124 | 69 supported, 19 unsupported, 26 unassessed | 50 supported, 38 unsupported, 26 unassessed | 114 unassessed | 114 unassessed | 14 supported, 3 unsupported, 97 unassessed |
...
The evidence corpus proves deterministic, static behavior of this local toolkit build. The toolkit does not execute SPL, evaluate regular expressions, compare result rows, model acceleration, load macro definitions, or certify runtime compatibility. The corpus also does not prove authorization or upstream product support.
...
- Registered expression functions now include exact two- or three-argument `mvindex`; exact two-argument `mvfind`, `relative_time`, `strftime`, and `like`; and zero-argument `true`, `null`, and `now`. Registered aggregate functions include exact one-argument `earliest`, `latest`, and `stdev`. Wrong arity and wrong aggregate context remain incomplete.
...
- `fillnull` supports an optional literal value and exact targets; all-fields mode preserves the current shape. `rex` supports default or exact input, a quoted pattern, literal `max_match`, exact `offset_field`, and unambiguous `(?<name>...)` or `(?P<name>...)` captures. `spath` requires default or exact input, an exact literal or dotted bare path, and an explicit exact output. `bin` and `bucket` support one exact input, optional exact alias, positive numeric `span` or `minspan` values without a unit, integer durations using recognized second, minute, hour, day, or month aliases, subsecond durations using `us`, `ms`, `cs`, or `ds` when the value divides evenly into one second and is shorter than one second, valid logarithmic `span` values, positive integer `bins`, numeric `start` or `end`, and `aligntime` as a time literal, `earliest`, or `latest`. `regex` supports a quoted expression against `_raw` or an exact field with `=` or `!=`. `mvexpand` supports one exact field. Dynamic fields and unsupported options remain incomplete, as do week, quarter, or year bin units, logarithmic `minspan`, `rex` sed mode or ambiguous captures, and `spath` auto-extraction.
- Ordinary child searches, join, and append use independent environments. Appendpipe inherits a copy. Join keys are parent reads. Child references, dependencies, diagnostics, transitions, and requirements remain visible, but child outputs do not leak into parent state. Branch merging remains incomplete and leaves known parent fields in an uncertain environment.
- `tstats` supports registered aggregates with exact optional aliases; a literal `datamodel=Model` or `datamodel=Model.Dataset` source; exact WHERE dependencies and field reads; exact BY groups; positive unsigned numeric `_time` spans without a unit or with an integer magnitude and a recognized second, minute, hour, day, or month alias; case-insensitive boolean values for `summariesonly`, `local`, `include_reduced_buckets`, and `allow_old_summaries`; a non-negative integer `chunk_size`; an exact literal `fillnull_value`; and exact `prestats=false` and `append=false`. Inline macros retain the direct macro and sibling facts, but expansion remains unresolved. Dynamic catalogs, wildcard groups, spans on non-`_time` groups, signed relative-time spans, malformed or quoted spans, zero spans, fractional spans with a unit, subsecond, week, quarter, year, logarithmic, snapped, or dynamic spans, `PREFIX(...)`, `prestats=true`, `append=true`, unknown options, and unknown output identities remain incomplete.
- Exact macro invocations retain one located macro dependency and direct requirement plus an unresolved-expansion gap. Macro bodies and transitive requirements are not loaded. Unknown commands remain opaque, and no default complete handler is assumed.

The minimized local detection corpus measures exact query bytes by syntax-complete case, semantic-complete selected target stage, and non-macro gap count. It improved from 12/1/35 to 14/16/13. Whole-query completeness is not an impact metric because a retained macro or branch boundary can correctly keep a query incomplete.
