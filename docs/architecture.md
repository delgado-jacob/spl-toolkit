...
Source Go builds may expose toolkit version `dev`; tagged CLI, server, native, and packaged builds use the exact `VERSION` while retaining the same semantic revision. The embedded corpus proves deterministic local toolkit behavior. It neither executes Splunk nor proves runtime equivalence, environment compatibility, authorization, or upstream support.
