...
func TestValidateFieldsRESTInputErrors(t *testing.T) {
	for _, route := range []string{"/api/v1/query/validate-fields", "/api/v1/query/validate-fields/batch"} {
		...
		for _, body := range []string{`null`, `{}`, `{"catalog":[]}`, valid + ` {}`, strings.Replace(valid, `"catalog":["host"]`, `"catalog":null`, 1), strings.Replace(valid, `"catalog":["host"]`, `"catalog":[],"catalog":[]`, 1), strings.Replace(valid, `"catalog":["host"]`, `"catalog":{"fields":[],"url":"https://example.com"}`, 1), strings.Replace(valid, `"text":"search host=x"`, `"text":null`, 1), strings.Replace(valid, `"text":"search host=x"`, `"text":"x","text":"y"`, 1), strings.Replace(valid, `"text":"search host=x"`, `"text":"x","unknown":true`, 1), strings.Replace(valid, `"text":"search host=x"`, `"text":"x","language":"spl2"`, 1), strings.Replace(valid, `"text":"search host=x"`, `"text":"\ud800"`, 1), strings.Replace(valid, `search host=x`, string([]byte{255}), 1), strings.Replace(valid, `["host"]`, `["host","host"]`, 1), strings.Replace(valid, `"catalog":`, `"unknown":`, 1)} {
