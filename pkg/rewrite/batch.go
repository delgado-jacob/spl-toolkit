...
func rewriteBatch(request BatchRequest,
	...
	documents := make([]analysis.QueryDocument, len(pending))
	for i, document := range prepared.Documents {
		pending[i], err = form(document, prepared.Rules, probes)
		if err != nil {
			return nil, err
		}
		documents[i] = pending[i].candidate.Session.Evidence().Analysis.Document
	}
	...
	if target := prepared.ValidationTarget; target != nil {
		if target.Kind == "field_list" {
			...
			for i, report := range batch.Reports {
				reports[i] = &CandidateValidation{Kind: target.Kind, FieldList: report}
			}
		} else {
			...
			for i, report := range batch.Reports {
				reports[i] = &CandidateValidation{Kind: target.Kind, Schema: report}
			}
		}
	}
