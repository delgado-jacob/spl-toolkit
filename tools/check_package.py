...
ACCEPTANCE_FILES = ("test_documented_cli.py", "test_surfaces.py", "test_analysis_surfaces.py", "test_validation_surfaces.py", "test_schema_surfaces.py", "cli_examples.json")
...
def toolkit_header_contract(payload: bytes) -> tuple[str, str]:
    """Keep the complete toolkit regions; cgo's compiler prologues vary by Go version."""
    markers = (
    ...
    # Toolkit declarations cannot be hidden in the excluded compiler-owned regions.
    if re.search(r"\b(?:spl_\w*|SPL\w*)\b", prefix + prologue):
        raise AssertionError("toolkit declaration in wheel header compiler prologue")
    def without_line_metadata(section: str) -> str:
        return re.sub(r'^#line [0-9]+ "[^"\r\n]*"\n', "", section, flags=re.MULTILINE)
    preamble, exports = without_line_metadata(preamble), without_line_metadata(exports)
    exports = re.sub(r"^(extern (?:int spl_mapper_new|char\* spl_toolkit_version))\(\);$",
                     r"\1(void);", exports, flags=re.MULTILINE)
    return preamble, exports
