...
Typed command operands remain outside safe-rewrite support. Held forms stay source-located and incomplete: inline macro expansion, dynamic catalog or field identities, wildcard `tstats` groups, spans on non-`_time` groups, signed relative-time spans, malformed or quoted spans, zero spans, fractional spans with a unit, subsecond, week, quarter, year, logarithmic, snapped, or dynamic `tstats` spans, unsupported bin units or subsecond magnitudes, logarithmic `minspan`, `PREFIX(...)`, `prestats=true`, `append=true`, unknown options or output identities, ambiguous captures, `rex` sed mode, `spath` auto-extraction, wrong function arity or context, and all branch merge effects.
