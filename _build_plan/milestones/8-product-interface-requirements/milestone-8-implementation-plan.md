...
Tasks 1 and 2 now define the public requirement value model and capture its private query-only evidence in the canonical traversal. The trace owns explicit event order and diagnostic links, remaps through the canonical reference and SPL2 stage maps, and remains byte-equivalent across finite, partial, wildcard, dotted-name, and divergent rename-conflict refinement cases. Reviewed regressions now prove query-local unavailability after rename, finite literal, and sound SQL projection effects; conditional provenance after SPL recovery, lookup `OUTPUTNEW`, partial dataset literals, deferred `bin` effects, hidden SQL reads, and rename conflicts; and exact diagnostic ownership for hidden SQL fields and multiple macros in one stage. Separate public and private conditionality, diagnostic incompleteness, rename conflicts, invalidation sets, and field updates prevent target evidence from changing the requirement trace. Rejected exact rename targets retain located reference evidence so later consuming references keep stable final IDs without applying rejected state. Wildcard rename targets invalidate concrete matches independently in the public and query-only snapshots, so closed literal fields do not survive unresolved substitution as definite values. The focused commands, race trace suite, full analysis and rewrite packages, vet, no-reentry scan, and diff check pass. `analysis.Result` still has no requirements field, and projection remains unimplemented as required by the Task 2 boundary. Tasks 3 through 8, whole-feature review, local acceptance, hosted CI, merge, and post-merge verification remain open.
