...
- [ ] (2026-09-14) Task 3 security-review reopening: A blocking resource-exhaustion review at `d1db37cf022bd1cbb640fa5ef0c9bcfc3c65f28b` supersedes the prior Task 3 specification and quality approvals for the range `d94451ca1ef2e37a8f754a1a60434c11672c0e3a..d1db37cf022bd1cbb640fa5ef0c9bcfc3c65f28b`. Keep every prior implementation SHA and verification result as historical evidence, but do not treat Task 3 as accepted. Implement the 4,096-unit canonical lexer budget, bounded resource-limit report, and O(1) trace indexes below, then rerun fresh specification, quality, and security reviews across the original Task 3 base through the new head.
...
Tasks 1 and 2 are accepted. The implementation through `d1db37c` defines the public requirement value model, captures private query-only evidence in the canonical traversal, and projects that finalized evidence into every `analysis.Result`. The standalone Go operation performs one `Analyze` call and returns a deep detached copy. Deterministic projection covers exact and conditional fields, all current direct knowledge kinds, wildcard and dynamic limitations, exact diagnostic ownership, independent query status and requirement coverage, stable digests, non-null collections, and omission of derived or query-local unavailable fields. A 17-case full requirement corpus covers valid, invalid, and incomplete SPL and SPL2, while all 34 current analysis reports and 14 transfer witness pairs carry the additive runtime field. The prior Task 3 functional and performance evidence remains valid historical evidence, but the security review supersedes its approvals until the canonical lexer budget, bounded result, trace indexes, and fresh reviews pass. The only deferred regression command remains the full rewrite package, whose Task 4-owned `explicitAliasReport` still has zero-valued hand-built requirements. Tasks 4 through 8, whole-feature review, local acceptance, hosted CI, merge, and post-merge verification remain open.
...
Canonical analysis admits at most 4,096 lexer work units for either language. Count every non-EOF token and every lexer error as separate units. Detect unit 4,097 before parser prediction and stop at its exact location. An over-budget normalized document makes `Analyze` return a non-nil report with a nil Go error. Its document is normalized and retains the full text; status is `incomplete`; syntax and semantic coverage are false with reason `SPL_ANALYSIS_RESOURCE_LIMIT`; stages, scopes, references, lineage, and every dependency array are initialized and empty; and its only diagnostic is a `warning` in category `resource_limit` with that code, a stable message, empty stage and scope IDs, and the first omitted token or error location. No evidence from the abandoned parse may survive.

The embedded and standalone requirement sets retain full normalized query identity and capability identity. The query digest covers the full supplied text. Query status is `incomplete`; coverage is false with only reason `SPL_ANALYSIS_RESOURCE_LIMIT`; items are empty; the only gap uses that code, has no reference IDs, and has `diagnostic_codes: ["SPL_ANALYSIS_RESOURCE_LIMIT"]`; and the same one diagnostic appears in the set. All collections are non-null arrays. `Requirements` returns the detached standalone set with a nil Go error. This behavior bounds canonical parsing across product surfaces, but it is not a universal serialized-result bound for inherited lineage amplification below the lexer budget. Long sparse input below the work budget remains admitted.
...
The canonical entry point wraps SPL and SPL2 lexing with the same 4,096-unit tracker. The tracker counts every non-EOF token and lexer error, captures the event that would consume unit 4,097, and returns a bounded resource-limit result before either parser predicts. That early result initializes every collection and computes capability and full-text query digests without constructing or retaining partial canonical evidence.
...
- [ ] Add failing exact-boundary tests for SPL and SPL2. Construct documents with exactly 4,096 lexer work units and require ordinary admission, then add one token or lexer error and require the bounded result. Include cases where lexer errors consume units and where the first omitted event is an error rather than a token.
- [ ] Implement one shared 4,096-unit lexer work tracker for both dialects. Count each non-EOF token and each lexer error, do not count EOF, capture the exact location of the event that would consume unit 4,097, and stop before parser prediction. Do not edit generated ANTLR sources.
- [ ] Add `SPL_ANALYSIS_RESOURCE_LIMIT` and a focused full-result test. Require normalized document identity, full supplied text, `status: incomplete`, false syntax and semantic coverage, one coverage reason, initialized empty stages, scopes, references, lineage, and dependency arrays, and exactly one `warning` diagnostic in category `resource_limit` with empty stage and scope IDs at the first omitted event. Assert that no partial parser, semantic, lineage, dependency, trace, or requirement evidence is emitted.
- [ ] Extend projector and standalone tests for the bounded result. Require full query and capability identity, a digest over the complete supplied text, incomplete requirement status, false coverage with only the resource-limit reason, empty items, one gap with no reference IDs and one matching diagnostic code, the same one diagnostic, non-null arrays, deep detachment, and deterministic repeated JSON.
- [ ] Add a long sparse input case whose byte length exceeds the dense witness while its lexer work stays at or below 4,096. Require ordinary parser admission so the work boundary cannot collapse into a hidden byte-size limit.
- [ ] Add O(1) indexes for pending-reference lookup and incomplete-stage membership in `requirementTrace`. Add compile-time or focused behavioral tests that require those indexes to stay synchronized through recording, remapping, and stage-completeness updates, and visible through environment clones, without changing existing ordering or serialized output.
- [ ] Add an end-to-end adversarial benchmark that analyzes dense wildcard documents, consumes the returned JSON, and reports time, allocations, and output bytes. Include the 65,533-byte witness and a 256 KiB witness; verify both stop at the same 4,097th work event with bounded empty evidence. Record benchmark measurements without inventing a universal serialized-output limit for admitted below-budget lineage.
- [ ] Run:
   ...
   Expected evidence is exact-limit admission, plus-one rejection before parser prediction, deterministic bounded output, full-text digests, lexer-error accounting, sparse-input admission, synchronized O(1) trace indexes, and a green analysis package. The existing Task 4-owned rewrite and current validation/schema fixture failures remain deferred; any other repository regression blocks review.
...
- [ ] Add over-budget preview and apply rewrite cases for original and candidate analysis. An over-budget original must stop before correspondence or candidate construction. An over-budget candidate must be rejected before proof or application. Neither path may accept rewrite transitions or partial requirement evidence from an abandoned parse.
...
- [ ] Add positive resource-limit analysis and requirement fixtures. Require `SPL_ANALYSIS_RESOURCE_LIMIT`, diagnostic category `resource_limit`, severity `warning`, incomplete statuses, false coverage, empty evidence arrays, one gap with no reference IDs, and full digest formats. Keep the existing v1 additive compatibility policy.
...
- [ ] Run concurrent resource-limit acceptance with the 64 KiB and 256 KiB cases. Require deterministic JSON and bounded canonical evidence without hangs, races, result aliasing, or native ownership failures. Include a long sparse input below 4,096 work units and require ordinary admission.
...
- [ ] Document the 4,096-unit lexer work boundary, token and lexer-error accounting, bounded incomplete result, cross-surface status behavior, unchanged 1 MiB REST request limit, and long sparse admission. State explicitly that this bounds canonical parsing but does not universally bound serialized inherited lineage below the lexer budget.
...
7. Both dialects count non-EOF tokens and lexer errors against the same 4,096-unit budget, stop before parser prediction at unit 4,097, emit no partial canonical evidence, and preserve the exact Go, CLI, REST, native, and Python content outcomes. Reviewers must also confirm that documentation does not misstate this parser bound as a universal serialized-result bound for below-budget lineage amplification.
...
6. Exact-limit, plus-one, lexer-error, full-digest, long-sparse, deterministic-output, 64 KiB, 256 KiB, and concurrency evidence proves that canonical parsing stops before prediction on unit 4,097 and every surface returns the specified bounded incomplete result without partial evidence. This is not presented as a universal serialized-result limit for inherited lineage below the lexer budget.
7. Permanent docs, source manifests, content manifests, and release manifests are complete; historical evidence is unchanged; local gates, independent reviews, Claude Opus review, independent acceptance, feature-branch CI, and post-merge `main` CI all pass at the exact accepted SHA.
