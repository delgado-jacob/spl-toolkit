...
Tasks 1 through 3 are accepted. Task 3 includes the public requirement model, query-only trace, deterministic projection, original 17-case corpus, 4,096-unit canonical lexer boundary, prompt abort of generated lexer error storms, limiter-local sentinel ownership, exact bounded incomplete result, and O(1) trace indexes. Its focused tests, complete analysis package, race gate, benchmark, and vet pass, and current analysis fixtures remain unchanged. Fresh specification, quality and architecture, and security and resource reviewers approved exact range `d94451ca1ef2e37a8f754a1a60434c11672c0e3a..1f94f8949642973aad63ebbb5f50a658fe317b96` with no actionable findings. Tasks 4 through 8 implementation and local verification are complete. Task 8 now extends that corpus to 18 cases so ordinary valid, invalid, and incomplete results are fixture-driven for both SPL and SPL2. Canonical requirements survive every refinement path, resource-limited rewrites return the exact fail-closed no-op report, all product adapters expose the same owned value, offline schemas and manifests publish the additive contract, and permanent documentation defines the product and security boundaries. The canonical corpus, dense and sparse resource cases, concurrent calls, strict request limits, executable CLI examples, source wheel, rebuilt sdist, release tooling, full Go check and race gates, generated header, and OpenAPI idempotence all pass locally. Installed evidence binds the exact copied requirement test bytes to current source. The removed milestone log and legacy Markdown and text files remain absent. Tasks 4 through 8 still require their independent review and acceptance steps. Whole-feature review, local independent acceptance, hosted exact-SHA CI, merge, and post-merge verification remain open and will be reported out of band after this pre-freeze commit.
...
- [x] Add 64 KiB and 256 KiB dense adversarial cases for SPL and SPL2 across both Go operations, both CLI operations, both real HTTP operations below the 1 MiB limit, native C, Python, installed wheel, and rebuilt sdist. Require the same bounded incomplete analysis or requirement value, CLI exit `3`, HTTP 200, owned native/Python success, full query digest, one diagnostic and gap, and empty partial evidence.
- [x] Run concurrent resource-limit acceptance with the ASCII dense 64 KiB and 256 KiB cases. Require deterministic JSON and bounded canonical evidence without hangs, races, result aliasing, or native ownership failures. For each fixture, serialized `RequirementSet` must be at most 4,096 bytes and serialized `Result` must be at most the full query byte length plus 4,096 bytes across repeated and concurrent runs. Include a long sparse input below 4,096 work units and require ordinary admission.
