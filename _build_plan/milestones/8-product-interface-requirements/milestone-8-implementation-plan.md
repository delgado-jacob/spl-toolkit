...
Tasks 1 through 3 now define the public requirement value model, capture private query-only evidence in the canonical traversal, and project that finalized evidence into every `analysis.Result`. The standalone Go operation performs one `Analyze` call and returns a deep detached copy. Deterministic projection covers exact and conditional fields, all current direct knowledge kinds, wildcard and dynamic limitations, exact diagnostic ownership, independent query status and requirement coverage, stable digests, non-null collections, and omission of derived or query-local unavailable fields. A 17-case full requirement corpus covers valid, invalid, and incomplete SPL and SPL2, while all 34 current analysis reports and 14 transfer witness pairs carry the additive runtime field. The exact Task 3 gate and full analysis package pass. The only deferred regression command is the full rewrite package, whose Task 4-owned `explicitAliasReport` still has zero-valued hand-built requirements. Tasks 4 through 8, whole-feature review, local acceptance, hosted CI, merge, and post-merge verification remain open.
