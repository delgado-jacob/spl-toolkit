...
- **Decision:** Preserve the CLI's positional and `--query` input contract. Use `strings.Repeat("a ", 4097)`, or its byte-identical fixture equivalent, for portable real-process resource-limit acceptance. It contains 4,097 identifier tokens, is 8,194 ASCII bytes and UTF-16 code units, and exceeds the 4,096-unit lexer work budget. Do not pass the 64 KiB, 256 KiB, or 300,000-byte sparse fixtures through CLI argv and do not add file, stdin, or batch acquisition. **Rationale:** Windows limits a process command line to 32,767 UTF-16 code units. This compact query and the complete executable command remain below that boundary. The operating-system transport limit does not change the canonical analyzer budget.
...
- [ ] Replace direct CLI argv use of the exact 64 KiB and 256 KiB dense fixtures with the byte-identical compact query `"a " * 4097` for both `analyze` and `requirements`, in both SPL and SPL2. Require CLI exit `3` and the exact resource-limit result. Assert that the fixture contains 4,097 identifier tokens, is exactly 8,194 ASCII bytes and UTF-16 code units, exceeds 4,096 lexer work units, and keeps the complete `subprocess.list2cmdline(args)` plus terminating NUL below the Windows 32,767-unit command-line boundary. Keep positional and `--query` as the only query inputs and retain the rejection of file, stdin, and batch options.
...
10. Real CLI process acceptance uses the compact 4,097-identifier, 8,194-code-unit query and keeps the complete command below the Windows 32,767 UTF-16 command-line boundary without adding file, stdin, or batch acquisition. The 64 KiB, 256 KiB, and 300,000-byte sparse fixtures never travel through CLI argv, and this transport constraint does not change the canonical analyzer budget.
...
6. Exact-limit, plus-one, real-error ordering, SPL2 closure, full-digest, exact range, 300 KB long-sparse, deterministic-output, exact 64 KiB, 256 KiB, and concurrency evidence proves that canonical parsing stops before parser construction or prediction on unit 4,097. Real CLI acceptance uses the compact 4,097-identifier, 8,194-code-unit query below the Windows argv limit. Go, HTTP, raw C, Python, installed wheel, and rebuilt sdist use the full dense and sparse fixtures. Every applicable transport returns the specified bounded incomplete result without partial evidence.
