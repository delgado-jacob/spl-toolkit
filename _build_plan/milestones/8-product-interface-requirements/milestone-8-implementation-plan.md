...
- [ ] (2026-09-15) Implement the Task 8 portability, exact-oracle, and native-helper corrections, then rerun Task 8 specification and quality review over the original Task 8 base through the new head.
...
Tasks 1 through 3 are accepted. Task 3 includes the public requirement model, query-only trace, deterministic projection, original 17-case corpus, 4,096-unit canonical lexer boundary, prompt abort of generated lexer error storms, limiter-local sentinel ownership, exact bounded incomplete result, and O(1) trace indexes. Its focused tests, complete analysis package, race gate, benchmark, and vet pass, and current analysis fixtures remain unchanged. Fresh specification, quality and architecture, and security and resource reviewers approved exact range `d94451ca1ef2e37a8f754a1a60434c11672c0e3a..1f94f8949642973aad63ebbb5f50a658fe317b96` with no actionable findings. Tasks 4 through 7 implementation and local verification are complete. Task 8 extended the corpus to 18 cases and completed its original local gates, but quality review reopened it for portable CLI coverage, a complete independent dense oracle, and subprocess isolation of dense native calls. The accepted amendment preserves every product contract and leaves the removed milestone log and legacy Markdown and text files absent. Task 8 implementation and rereview remain open. Whole-feature review, local independent acceptance, hosted exact-SHA CI, merge, and post-merge verification cannot begin until those corrections pass.
...
- [ ] Run the corrected focused gate:
...
- [ ] Rerun fresh installed-wheel and rebuilt-sdist acceptance through `python3 -m pytest -q tools/tests/test_package.py tools/tests/test_acceptance.py tools/tests/test_release.py`. Require both installed paths to execute the corrected hash-bound acceptance file, including its helper subprocess mode, without checkout imports or source-path injection.
- [ ] Rerun the repository-level local gates, update `Progress`, `Surprises & Discoveries`, `Decision Log`, and `Outcomes & Retrospective`, then commit the Task 8 correction with `git commit -m "test(acceptance): make dense probes portable"`. Do not restore the milestone log or any removed Markdown or text file.
