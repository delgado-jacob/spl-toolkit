...
- **Decision:** On every conflicting or unsupported rename, delete the same source and destination names from the public and query-only field maps, then retain the diagnostic-owned uncertainty in both environments. **Rationale:** The rename outcome is unknown, so a later read is conditional and indeterminate rather than a definite derived field or an unavailable tombstone.
...
Tasks 1 and 2 now define the public requirement value model and capture its private query-only evidence in the canonical traversal. The trace owns explicit event order and diagnostic links, remaps through the canonical reference and SPL2 stage maps, and remains byte-equivalent across finite, partial, wildcard, and dotted-name refinement cases. Reviewed regressions now prove query-local unavailability after rename, finite literal, and sound SQL projection effects; conditional provenance after SPL recovery, lookup `OUTPUTNEW`, partial dataset literals, deferred `bin` effects, hidden SQL reads, and rename conflicts; and exact diagnostic ownership for hidden SQL fields and multiple macros in one stage. The first exact conditional-field RED command failed all three requested rows because the trace classified their final reads as derived and nonconditional. A later refinement-isolation RED run failed the rename, aggregate, and lookup destination witnesses because public conditionality reached their private output fields. Repository-wide validation also caught the hidden-HAVING rewrite contract losing `ref-2`, `ref-3`, and `ref-4`; separate public and private diagnostic incompleteness now preserves those rewrite references while retaining incomplete trace evidence for the hidden field. The quality follow-up proved that a conflicting rename could leave the private `a` field derived after public invalidation, and that the variadic trace argument did not enforce exactly one call-site choice. The focused commands, race trace suite, full analysis and rewrite packages, vet, no-reentry scan, and diff check pass. `analysis.Result` still has no requirements field, and projection remains unimplemented as required by the Task 2 boundary. Tasks 3 through 8, whole-feature review, local acceptance, hosted CI, merge, and post-merge verification remain open.
