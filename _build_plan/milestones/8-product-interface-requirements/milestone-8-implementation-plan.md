...
- An additional repository-wide test run found the sandbox blocks the existing API `httptest` listener and `pkg/rewrite` has a `sql-having-conflict-hidden` corpus mismatch. A detached worktree at the Task 2 starting SHA `dc6c9e83d38db9408b86c5068b6f62305e6b6828` reproduced the rewrite mismatch, so this scoped fix did not introduce it.
...
Tasks 1 and 2 now define the public requirement value model and capture its private query-only evidence in the canonical traversal. The trace owns explicit event order and diagnostic links, remaps through the canonical reference and SPL2 stage maps, and remains byte-equivalent across finite, partial, wildcard, and dotted-name refinement cases. Reviewed regressions now prove query-local unavailability after rename, finite literal, and sound SQL projection effects; conditional provenance after SPL recovery, lookup `OUTPUTNEW`, partial dataset literals, deferred `bin` effects, and hidden SQL reads; and exact diagnostic ownership for hidden SQL fields and multiple macros in one stage. The first exact conditional-field RED command failed all three requested rows because the trace classified their final reads as derived and nonconditional. A later refinement-isolation RED run failed the rename, aggregate, and lookup destination witnesses because public conditionality reached their private output fields. The same focused commands, the full trace suite, and the complete analysis package pass after separating public and query-only expression and output conditionality. `analysis.Result` still has no requirements field, and projection remains unimplemented as required by the Task 2 boundary. Tasks 3 through 8, whole-feature review, local acceptance, hosted CI, merge, and post-merge verification remain open.
