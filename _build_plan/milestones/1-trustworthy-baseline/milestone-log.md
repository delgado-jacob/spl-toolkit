...
- REST mapping configuration is request-scoped by default. The process-global mapping endpoint remains available only when explicitly enabled for development compatibility.
...
- Flat `InputFields` remains available for compatibility even though it cannot express lineage or read/write roles. Unsupported macro recovery returns known macros and leaves the other categories empty instead of presenting partial data as complete analysis.
