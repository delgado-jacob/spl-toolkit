...
The 12 approved design completion criteria map to passing evidence as follows:

1. The canonical ledger covers every approved record kind for SPL and SPL2, and every evidence case is referenced. `pkg/analysis/capabilitydata/ledger.json`, `pkg/analysis/capabilitydata/corpus.json`, `TestCapabilityLedgerCoversEveryKindPerLanguage`, and `TestCapabilityEvidenceIsReferenced` passed. The built CLI reported 68 SPL records with 67 evidence cases and 98 SPL2 records with 108 evidence cases.
2. Every record has exactly the five syntax, semantics, requirements, linting, and safe-rewriting claims. `TestEmbeddedCapabilityAssetsAreStructurallyValid`, the strict `decodeCapabilityAssets` path, and `test_capability_ledger_shapes_are_strict_and_optional` passed.
3. State-specific evidence rules hold for `supported`, `partial`, and `unsupported` claims. `not_applicable` claims require a reason and no evidence; `unassessed` claims receive no evidence or covered credit. `TestValidateCapabilityClaims`, `TestCapabilitySummaryUsesStrictDenominator`, and the external `TestCapabilityEvidenceCorpus` execution passed.
4. Canonical positive, negative, and incomplete evidence IDs remain stable, ordered, unique, and structurally valid. `pkg/analysis/capabilitydata/corpus.json`, `TestCapabilityDataCanonicalOrder`, `TestDecodeCapabilityAssetsRejectsMalformedInput`, `TestDecodeCapabilityAssetsRejectsMalformedObservations`, and `TestCapabilityEvidenceCorpus` passed.
5. Parser registration remains separate from evidence-backed syntax coverage. `TestCapabilityGrammarRegistrationDoesNotAddSyntaxCoverage` passed for `spl2.command.spl1.quoted-pipeline`, which is registered, unsupported, and receives zero covered syntax credit. Current SPL and SPL2 manifests report every linting record as unassessed.
6. Summary counts match the selected records on every public surface, use strict denominators, and publish no percentage or composite score. `TestCapabilitySummaryUsesStrictDenominator`, `TestCapabilitiesPublishCanonicalLedger`, `test_capability_ledger_contract_and_additive_v1_compatibility`, and `test_analysis_capabilities_parity` passed.
7. Existing public surfaces retain their exact shape and legacy projections. `TestCapabilitiesPreserveLegacyProjection`, `TestDialectCapabilitiesCLIHTTPParity`, `TestCapabilitiesRESTMatchesKernel`, `TestCapabilitiesBindingsReturnCompleteCanonicalOwnedManifests`, and `test_analysis_capabilities_parity` passed. The `main..HEAD` review found no new endpoint, CLI command, C export, or Python method.
8. Tagged version propagation is exact. The built CLI reported `toolkit_version: 0.1.1`; `TestCapabilitiesPublishCanonicalLedger`, `test_source_capability_version_is_independent_of_package_and_native`, `test_analysis_capabilities_parity`, and `make python-test` passed for source `dev` isolation and tagged CLI, server, native, wheel, and rebuilt-sdist artifacts.
9. The semantic revision covers the typed capability contract and excludes only `toolkit_version`. SPL and SPL2 revisions are `sha256:1e6c75800f843931ec517dba27f3baa5af928a8a908d97dd1c62513e2ea24d31` and `sha256:0203cbeec2e0fd484080b1f512582a1c8bdc4bc541fb73ac42c013060695f84c`. `TestCapabilityRevisionUsesNormalizedTypedManifest` and `TestAnalysisRevisionMatchesCanonicalManifestPayload` passed.
10. Archived v1 manifests remain valid while new ledger members are strict when present. The `capabilities-archived-v1` artifact in `testdata/tooling/contracts.json`, `test_capability_ledger_contract_and_additive_v1_compatibility`, `test_capability_ledger_shapes_are_strict_and_optional`, and the semantic JSON/YAML/docs.go reconciliation check passed. Generated OpenAPI changes remain paired with `tools/update_validation_openapi.py` and its tests.
11. Source, wheel, and rebuilt-sdist closures carry the same ledger, corpus, tests, schemas, and tagged native behavior without a checkout dependency. `python/native-source-files.txt`, `tools/release-source-files.txt`, `test_native_source_manifest_contains_capability_ledger_closure`, `test_capability_ledger_files_are_release_source_inputs_only`, `test_copied_capability_tests_do_not_require_checkout_version`, and `make python-test` passed. Production data is compiled through `go:embed`; the full diff review found no runtime external checkout, network, live Splunk, Git, or worktree read.
12. Maintained documentation and the milestone record describe the public contract and its limits. The 11 capability documentation files listed above, this milestone log, `python3 tools/check_docs.py`, and `git diff --check` passed. The full diff review found no parser or grammar change, adapter classification, historical `docs/evidence` refresh, percentage, score, or composite score.
